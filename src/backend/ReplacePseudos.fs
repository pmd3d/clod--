...
let calculateOffset state name =
...
let replaceOperand state = function
    (* if it's a pseudoregister, replace it with a stack slot *)
    | Pseudo s ->
        if AssemblySymbols.isStatic s then (state, Data (s, 0))
        else
            ...
            | None ->
                let new_state, new_offset = calculateOffset state s
                (new_state, Memory (BP, new_offset))
    | PseudoMem (s, offset) when AssemblySymbols.isStatic s ->
        (state, Data (s, offset))
    | PseudoMem (s, offset) ->
        match Map.tryFind s state.offset_map with
        (* We've already assigned this operand a stack slot *)
        ...
        | None ->
            (* assign s a stack slot, and add its offset to the offset w/in s to
               get new operand *)
            let new_state, new_var_offset = calculateOffset state s
            (new_state, Memory (BP, offset + new_var_offset))
    (* not a pseudo, so nothing to do *)
    | other -> (state, other)

let replacePseudosInInstruction state = function
    (* Replace src and dst of mov instruction *)
    | Mov (t, src, dst) ->
        let state1, new_src = replaceOperand state src
        let state2, new_dst = replaceOperand state1 dst
        let new_mov = Mov (t, new_src, new_dst)
        (state2, new_mov)
    | Movsx fields ->
        let state1, new_src = replaceOperand state fields.src
        let state2, new_dst = replaceOperand state1 fields.dst
    ...
    | Unary (t, op, dst) ->
        let state1, new_dst = replaceOperand state dst
    ...
    | Binary { op = op; t = t; src = src; dst = dst } ->
        ...
        (state2, new_binary)
    | Cmp (t, op1, op2) ->
        let state1, new_op1 = replaceOperand state op1
        let state2, new_op2 = replaceOperand state1 op2
        let new_cmp = Cmp (t, new_op1, new_op2)
        (state2, new_cmp)
    | Idiv (t, op) ->
        let state1, new_op = replaceOperand state op
...
let replacePseudosInTl = function
    | Function { name = name; ``global`` = ``global``; instructions = instructions } ->
        ...
        let map_folder s i =
            let s', i' = replacePseudosInInstruction s i
            (i', s')

        let fixed_instructions, final_state =
            List.mapFold map_folder init_state instructions
        
        AssemblySymbols.setBytesRequired name final_state.current_offset
        Function { name = name; ``global`` = ``global``; instructions = fixed_instructions }
    | static_var -> static_var

let replacePseudos (Program tls) =
    let fixed_defs = List.map replacePseudosInTl tls
    Program fixed_defs
